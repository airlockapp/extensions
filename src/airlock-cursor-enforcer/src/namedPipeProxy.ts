...
export class NamedPipeProxy {
    ...
    private async _processRequest(socket: net.Socket, raw: string): Promise<void> {
        ...
        //     (workspace/action-level DND can auto-approve/deny without mobile interaction)
        //     Same gatewayUrl is used for DND policy fetch and DND audit POST — do not run when empty.
        if (commandLine && this._opts.gatewayUrl?.trim()) {
            try {
    ...
    /**
     * Submit a short-lived artifact for DND-bypassed commands so they are visible
     * in mobile history. Uses the same gateway URL as the normal flow (this._opts.gatewayUrl).
     */
    private async _submitDndAuditArtifact(
        ...
        try {
            ...
            // Short-lived artifact: let it expire quickly so it moves to history
            // but still satisfies gateway validation (expiresAt must be in future).
