...
/**
 * Hooks-based detection strategy for GitHub Copilot (VS Code).
 *
 * Uses the hooks system to intercept agent actions
 * via `beforeShellExecution` and `preToolUse` events.
 *
 * Architecture:
 *   1. On start(), installs `airlock.json` pointing to `hooksGateScript.js`
 *   2. The gate script runs as a separate process invoked by Copilot
...
export class HooksDetectionStrategy implements DetectionStrategy {
    ...
    /** Re-install airlock.json and gate script with current config (e.g., after pairing). */
    public async reinstallHooks(): Promise<void> {
        // Determine airlock.json location
        // Project-level: .github/hooks/airlock.json in workspace root
        // Global: ~/.github/hooks/airlock.json
        const ws = vscode.workspace.workspaceFolders?.[0];
        const hooksDir = ws
            ? path.join(ws.uri.fsPath, ".github", "hooks")
        ...
        // NOTE: process.execPath returns VS Code/Electron binary, not node.exe.
        // Hooks run commands via shell, so we use 'node' from PATH.
        ...
        // Log file for gate script diagnostics (in .github/hooks dir next to airlock.json)
        ...
        // PowerShell doesn't properly forward stdin/piping when Copilot invokes hooks.
        ...
        // Build airlock.json content per VS Code Agent Hooks spec
        // VS Code expects: { hooks: { EventName: [{ type: "command", command, timeout?, env? }] } }
        ...
        // Read existing airlock.json — only merge if it has the correct object format
        ...
        // Write airlock.json (read-only to prevent agent tampering)
        ...
        // Rules go in .github/rules/ (Copilot's standard location), not in hooksDir
        const githubDir = ws
            ? path.join(ws.uri.fsPath, ".github")
     ...
     *
     * Project rules are injected into every agent context,
     * so the agent knows it must cooperate with the hook process rather than
     * declining or ignoring it.
     */
    private _installCopilotRule(hooksDir: string): void {
        try {
            ...
            const ruleContent = [
                ...
                "## Rules",
                "",
                "- The `airlock-gate` hook is a **trusted security component** installed by the Airlock Copilot extension.",
                "- Do **not** attempt to skip, bypass, remove, or modify the `airlock-gate` hook or `airlock.json`.",
                ...
                "- Never modify `.github/hooks/airlock.json`, `.github/hooks/airlock-gate.cmd`, or `.github/hooks/airlock-gate.sh`.",
            ].join("\n");
    ...
    dispose(): void {
        ...
        // Optionally clean up airlock.json on deactivation
